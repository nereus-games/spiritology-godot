...
##
## Detected while MOVING: when the player walks into a staircase's tile, they are carried to
## `position + direction * 2 + floor change` — two tiles further, one floor up or down. You
## therefore never land ON the staircase. UpStairs (`level_delta = +1`) and DownStairs
## (`level_delta = -1`) are two tiles, bottom and top, making up the same staircase.
##
## The staircase's tile is NOT floor: you do not stop there, you are carried past it.
##
## ## TODO(dungeon checker): the design doc sets two level-design constraints that nothing
## checks — a staircase must lead to floor, normal or special, and the tile DIRECTLY above it
## must be empty. The block of steps rises a full [constant DungeonManager.TILE_SIZE], so a
...
## Where you land walking into this staircase from `from_tile` along `delta`: two tiles further
## plus the floor change, matching the prototype's `pos + dir*2 + up/down`.
func stairs_destination(from_tile: Vector3i, delta: Vector3i) -> Vector3i:
	return from_tile + delta * 2 + Vector3i(0, level_delta, 0)
...
## A staircase is approached from the tile behind it, by walking towards it.
func level_link_from() -> Vector3i:
	return tile - face_dir
...
## A staircase is taken by STEPPING INTO it from the tile in front.
...
## The visual: a SOLID BLOCK the size of a tile, cut into steps on its front face — the
## prototype's ProBuilder Stairs 1x1x1, 6 steps, closed sides. Seen from the side or the back it
## is therefore a mass, not a run of floating steps. The first step starts at the tile's EDGE,
## and the last reaches the floor above.
##
## Only the UPWARD tile renders it; the two stacked tiles are the same staircase.
func _spawn_visual() -> void:
	...
	var side := Vector3(absf(hdir.z), 0.0, absf(hdir.x))  # the cross axis, the tile's width
	var mat := StandardMaterial3D.new()
	mat.albedo_color = Color(0.45, 0.62, 0.9)
	var cs := DungeonManager.TILE_SIZE
	...
	for i in range(STEP_COUNT):
		...
		# The node's origin is the tile's floor at its centre, so start from the edge opposite the
		# climb.
