...
## onto the player's tile (or the other way round) starts an encounter. Turns arrive through
...
## doc's "Visuals + Sounds" caps rival sprites at 90 cm x 90 cm. A tile is
## [constant DungeonManager.TILE_SIZE] (1 m), so 0.9 fills most of the tile without ever
...
func _fit_sprite() -> void:
	...
	# tall (jézal, 2000 x 1898) spill out of the tile sideways.
	...
	# the tile's floor instead of half-buried in it (or floating above).
...
func take_turn(player_tile: Vector3i) -> void:
	...
	# (carried two tiles further, plus a floor change). From the wrong side they block like a
	# wall.
	...
	if _dungeon.is_narrow_bridge(next):
		...
		if occupant != null and occupant != self:
			...
			return
	elif next == player_tile:
		_dungeon.request_encounter(self, true)  # contact starts an encounter, with no move
		return

	if _dungeon.move_occupant(tile, next, self):
		await _step_to(next)
		if _dungeon == null:
			return  # dissolved in the meantime
		_dungeon.notify_entered(next, self)  # traps on the tile it reached
...
func _update_memory(player_tile: Vector3i) -> void:
	if _can_see(player_tile):
		_target_tile = player_tile
		_has_target = true
		_memory = memory_turns
	elif _memory > 0:
		_memory -= 1
...
## tiles between rival and player and cut the line at the first opaque edge
## ([method DungeonManager.is_edge_opaque]) or non-walkable tile. Revisit with real dungeons,
## where occlusion actually matters.
func _can_see(player_tile: Vector3i) -> bool:
	# The player is hidden (fog mantel, torment veil, a costume): undetectable.
	if _dungeon != null and _dungeon.rivals_ignore_player():
		return false
	if player_tile.y != tile.y:
		return false
	var d: Vector3i = (player_tile - tile).abs()
	return d.x + d.z <= vision_range


func _chase_dir(target: Vector3i) -> Vector3i:
	var diff := target - tile
...
func _step_to(next: Vector3i) -> void:
	...
	tween.tween_property(self, "global_position", _dungeon.tile_to_world(next), move_duration)
...
func witness_player_level_change(from_tile: Vector3i, to_tile: Vector3i) -> void:
	if not _can_see(from_tile):
		return
	_level_target = to_tile
...
## target tile has been set to the staircase to reach.
##
...
func _pursue_level() -> bool:
	var going_down: bool = _level_target.y < tile.y
	if going_down:
		...
		if brink != tile:
			...
			if landing.y >= _level_target.y:
				if randf() >= fall_pursuit_chance(tile.y - landing.y):
	...
	if tile == approach:
		...
		return false
	_target_tile = approach
	_has_target = true
	return false


## The most promising open edge to jump towards `toward`: a neighbouring tile over empty space,
## whose edge nothing bars (a GUARDRAIL protects it just as it protects the player), with floor
## somewhere below. Returns the current tile when no edge will do, and the rival stays on its
...
func _open_drop_edge(toward: Vector3i) -> Vector3i:
	...
	for d in [Vector3i(1, 0, 0), Vector3i(-1, 0, 0), Vector3i(0, 0, 1), Vector3i(0, 0, -1)]:
		...
		if _dungeon.is_edge_blocked(tile, n) or _dungeon.is_floor(n):
			continue
...
## Takes the `link` from the current tile — a staircase to climb, and so on. Returns false when
...
## A fall from `from_tile`, chosen or not: lands on the floor below and takes the damage.
## Returns false when the rival is devitalised by it, and therefore dissolved.
func fall_down(from_tile: Vector3i) -> bool:
	var landing: Vector3i = _dungeon.fall_landing(from_tile)
	if landing == from_tile:
		...
		if _dungeon.is_bottomless(from_tile):
			queue_free()
			return false
		return true
	return drop_to(landing, from_tile.y - landing.y)


## Puts the rival on `target` at the end of a fall of `levels` floors and applies the damage.
## When `target` is occupied it lands on the nearest free tile instead: the one-occupant-per-
## tile invariant holds across the whole grid, and a fall is no reason to bend it. Returns
## false when the fall devitalises it.
func drop_to(target: Vector3i, levels: int) -> bool:
	var dest: Vector3i = _dungeon.free_tile_near(target)
	_dungeon.release(tile)
	_dungeon.reserve(dest, self)
	tile = dest
	# The tile changes at once, because the caller needs the result within the current turn, but
	# the descent is ANIMATED: without that the rival vanishes off the plank and reappears below,
	# and you never see that it fell.
	_busy = true
	var tween := create_tween()
	tween.tween_property(self, "global_position", _dungeon.tile_to_world(dest), 0.35)
...
## Two rivals meet on a plank: both fall, per the design doc — the occupant onto the tile
...
func _collide_with_rival(tile: Vector3i, other: Node) -> void:
	...
	drop_to(_dungeon.free_tile_near(landing), levels)
