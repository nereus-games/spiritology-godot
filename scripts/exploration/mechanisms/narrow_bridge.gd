## One narrow-bridge tile — a log over a drop — and its balance test.
##
## Per the design doc the character ADVANCES ON ITS OWN, and the player corrects in real time
## with lateral moves to stay upright. Every tile of the bridge carries this mechanism: stepping
## onto one engages a balance test that advances ONE tile along the direction being FACED, so a
## bridge works in both directions. The pure balance logic lives in [NarrowBridgeBalance]; the
## real-time driver — lateral input, camera roll, chaining tile to tile, falling — is in
## `exploration.gd`.
##
...
## Engages the balance test to advance ONE tile along `dir`.
func engage(dir: Vector3i, disarrayed: bool) -> void:
	...
	_balance.configure(GameSession.psy_score, 1, disarrayed)  # one tile
	_engaged = true


## Picks up the balance test ALREADY UNDER WAY, from a crossing started on the previous tile,
## rather than starting a fresh one: imbalance and lateral velocity carry across the tile
...
## The balance model itself; the driver reads and writes imbalance through it to chain tiles.
func balance():
	return _balance
