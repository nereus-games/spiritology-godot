...
signal ifp_earned(species_id: StringName, action: GameEnums.IfpAction, is_forlorn: bool, dialogue_effective: bool)
...
func setup(player_fighters: Array, rival_fighters: Array, seed: int = 0, completed: Dictionary = {}) -> void:
...
func is_offensive(ability: AbilityData) -> bool:
	return ability.tags.has("damage") or ability.tags.has("ETH loss") \
		or ability.tags.has("change weakness") or ability.tags.has("limit actions")
...
func menu_kinds(fighter: EncounterFighter) -> Array:
	var kinds: Array = [EncounterAction.Kind.ABILITY, EncounterAction.Kind.TALK, EncounterAction.Kind.EXAMINE]
	for t in _talents:
		if t.owner == fighter:
...
func _award_dissolutions(dissolved_before: Array) -> void:
	for r in rivals:
		if r.is_dissolved() and not dissolved_before.has(r):
			ifp_earned.emit(r.species_id(), GameEnums.IfpAction.DISSOLVE_RIVAL, _is_forlorn(r), true)
...
func _resolve_object(fighter: EncounterFighter, action: EncounterAction) -> void:
	var obj: ObjectData = object_provider.call(action.object_id) if object_provider.is_valid() else null
	if obj == null:
	...
	match obj.effect:
		GameEnums.ObjectEffect.HEAL_DEN:
			...
			lines.append("%s %s à %s : DEN %d -> %d." % [
				"donne" if given else "utilise", tr(obj.name_key()), target.display_name(), before, target.den])
		GameEnums.ObjectEffect.FLEE_ENCOUNTER:
			...
			lines.append("%s %s : fuite (TODO exploration)." % [
...
func _resolve_steal(fighter: EncounterFighter, action: EncounterAction) -> void:
	...
	if target == null:
		...
		return
	_emit_turn(fighter, action, ["tente de voler %s [objet volé / Coal Vetch à venir]." % target.display_name()])
...
func _resolve_meditate(fighter: EncounterFighter, action: EncounterAction) -> void:
	...
	_emit_turn(fighter, action, ["médite : ETH %d -> %d, dégâts +%d %% au prochain tour."
...
func _resolve_talk(fighter: EncounterFighter, action: EncounterAction) -> void:
	...
	ifp_earned.emit(target.species_id(), GameEnums.IfpAction.TALK_RIVAL, _is_forlorn(target), effective)
	_emit_turn(fighter, action, ["parle à %s : dialogue %s." %
...
func _resolve_examine(fighter: EncounterFighter, action: EncounterAction) -> void:
	...
	if target == null:
		...
		return
	ifp_earned.emit(target.species_id(), GameEnums.IfpAction.EXAMINE_RIVAL, _is_forlorn(target), true)
	...
	if ability.single_use:
		for t in _talents:
			if t.owner == fighter and t.wants_reuse(self, fighter, ability):
				...
				note_talent("%s : %s redevient disponible (Reuse)." % [fighter.display_name(), ability.id])
				break
...
func _weakness_touched_targets(_user: EncounterFighter, ability: AbilityData, targets: Array) -> Array:
