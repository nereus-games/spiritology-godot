...
## [method flee_eth_cost].
## ## TODO: the doc puts a QTE behind that Run Away, so the attempt can fail. No QTE system
## exists, so for now it always succeeds — see [method EncounterManager._resolve_flee].
